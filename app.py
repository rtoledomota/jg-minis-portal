...
def load_from_google_sheets():
    try:
        ...
        for line in lines[1:]:
            ...
            if parts[idx_imagem] and parts[idx_nome]:
                try:
                    qtd = int(parts[idx_qtd]) if parts[idx_qtd] else 0
                    valor = float(parts[idx_valor]) if parts[idx_valor] else 0.0
                    max_res = int(parts[idx_max]) if parts[idx_max] else 3
                    
                    miniaturas.append((
                        convert_drive_url(parts[idx_imagem]),
                        parts[idx_nome],
                        parts[idx_chegada],
                        qtd,
                        valor,
...
def init_db():
    ...
    c.execute('''CREATE TABLE users (id INTEGER PRIMARY KEY, email TEXT UNIQUE, password TEXT, phone TEXT, is_admin BOOLEAN, created_at TEXT)''')
    c.execute('''CREATE TABLE miniaturas (id INTEGER PRIMARY KEY, image_url TEXT, name TEXT, arrival_date TEXT, stock INTEGER, price REAL, observations TEXT, max_reservations_per_user INTEGER)''')
    c.execute('''CREATE TABLE reservations (id INTEGER PRIMARY KEY, user_id INTEGER, miniatura_id INTEGER, quantity INTEGER, created_at TEXT, FOREIGN KEY(user_id) REFERENCES users(id), FOREIGN KEY(miniatura_id) REFERENCES miniaturas(id))''')
...
LOGIN_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>JG MINIS</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"></head><body class="bg-gradient-to-br from-orange-500 via-yellow-500 to-orange-600 min-h-screen flex items-center justify-center"><div class="w-full max-w-md"><div class="bg-white rounded-2xl shadow-2xl overflow-hidden"><div class="bg-gradient-to-r from-orange-500 to-yellow-500 p-8 text-center"><div class="w-24 h-24 bg-white rounded-xl flex items-center justify-center mx-auto mb-4 shadow-lg"><img src="{{ logo_url }}" alt="Logo" class="w-20 h-20 object-contain"></div><h2 class="text-3xl font-bold text-white mb-2">JG MINIS</h2><p class="text-orange-100">Portal de Miniaturas</p></div><div class="p-8"><div class="flex gap-4 mb-6 border-b"><button class="nav-tab active pb-4 px-4 font-bold text-orange-600 border-b-2 border-orange-600 cursor-pointer" onclick="switchTab('login')">Login</button><button class="nav-tab pb-4 px-4 font-bold text-gray-500 cursor-pointer" onclick="switchTab('register')">Cadastro</button></div><div id="login" class="tab-content">{% if error %}<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg mb-4">{{ error }}</div>{% endif %}<form method="POST" action="/login"><div class="mb-4"><label class="block text-gray-700 font-bold mb-2">Email</label><input type="email" name="email" class="w-full border-2 border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-orange-500" required></div><div class="mb-6"><label class="block text-gray-700 font-bold mb-2">Senha</label><input type="password" name="password" class="w-full border-2 border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-orange-500" required></div><button type="submit" class="w-full bg-gradient-to-r from-orange-500 to-yellow-500 text-white font-bold py-3 rounded-lg hover:shadow-lg transition">Entrar</button></form><hr class="my-4"><p class="text-sm text-gray-600"><strong>Teste:</strong><br>admin@example.com / admin123</p></div><div id="register" class="tab-content hidden">{% if register_error %}<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg mb-4">{{ register_error }}</div>{% endif %}{% if register_success %}<div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg mb-4">{{ register_success }}</div>{% endif %}<form method="POST" action="/register"><div class="mb-4"><label class="block text-gray-700 font-bold mb-2">Email</label><input type="email" name="email" class="w-full border-2 border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-orange-500" required></div><div class="mb-4"><label class="block text-gray-700 font-bold mb-2">Telefone</label><input type="tel" name="phone" class="w-full border-2 border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-orange-500" required></div><div class="mb-4"><label class="block text-gray-700 font-bold mb-2">Senha</label><input type="password" name="password" class="w-full border-2 border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-orange-500" minlength="6" required></div><div class="mb-6"><label class="block text-gray-700 font-bold mb-2">Confirme a Senha</label><input type="password" name="confirm_password" class="w-full border-2 border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-orange-500" minlength="6" required></div><button type="submit" class="w-full bg-gradient-to-r from-orange-500 to-yellow-500 text-white font-bold py-3 rounded-lg hover:shadow-lg transition">Cadastrar</button></form></div></div></div></div><script>function switchTab(tab){document.querySelectorAll('.tab-content').forEach(e=>e.classList.add('hidden'));document.getElementById(tab).classList.remove('hidden');document.querySelectorAll('.nav-tab').forEach(e=>e.classList.remove('border-b-2','border-orange-600','text-orange-600'));event.target.classList.add('border-b-2','border-orange-600','text-orange-600')}</script></body></html>'''

HOME_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>JG MINIS</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"></head><body class="bg-gray-50"><nav class="bg-gradient-to-r from-orange-500 to-yellow-500 shadow-lg sticky top-0 z-50"><div class="container mx-auto px-4 py-4 flex justify-between items-center"><a href="/" class="flex items-center gap-3"><div class="w-12 h-12 bg-white rounded-lg flex items-center justify-center shadow-md"><img src="{{ logo_url }}" alt="Logo" class="w-10 h-10 object-contain"></div><span class="text-2xl font-bold text-white">JG MINIS</span></a><div class="flex items-center gap-4">{% if is_admin %}<a href="/admin" class="bg-white text-orange-600 font-bold px-4 py-2 rounded-lg hover:shadow-lg transition"><i class="fas fa-chart-bar mr-2"></i>Admin</a>{% endif %}<span class="text-white font-semibold"><i class="fas fa-user mr-2"></i>{{ email }}</span><a href="/logout" class="bg-red-600 text-white font-bold px-4 py-2 rounded-lg hover:bg-red-700 transition">Sair</a></div></div></nav><div class="container mx-auto px-4 py-12"><h1 class="text-4xl font-bold text-gray-900 mb-2"><i class="fas fa-cube text-orange-500 mr-3"></i>Catálogo de Miniaturas</h1><p class="text-gray-600 mb-8">Explore nossa coleção exclusiva</p><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{{ miniaturas|safe }}</div></div></body></html>'''

ADMIN_HTML = '''<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>JG MINIS - Admin</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"></head><body class="bg-gray-50"><nav class="bg-gradient-to-r from-orange-500 to-yellow-500 shadow-lg"><div class="container mx-auto px-4 py-4 flex justify-between items-center"><h1 class="text-2xl font-bold text-white"><i class="fas fa-shield mr-2"></i>JG MINIS - Admin</h1><a href="/logout" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700">Sair</a></div></nav><div class="container mx-auto px-4 py-8"><div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">{{ stats|safe }}</div><div class="bg-white rounded-lg shadow-lg p-6">{{ content|safe }}</div></div></body></html>'''
