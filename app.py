...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'a_very_secret_key_for_jg_minis_v4_3_2_stable')
...
def init_db():
    ...
    if not conn:
        logging.error('Falha ao obter conexão DB para init_db. Verifique permissões/caminho.')
        return
    c = conn.cursor()
    try:
        ...
        if not c.fetchone():
            ...
            conn.commit() # Commit admin creation immediately
        ...
        if c.fetchone()[0] == 0 and sheet:
            ...
            logging.warning('Não foi possível fazer sync inicial: Google Sheets não disponível.')
...
def sync_stock_from_sheet():
    if not sheet:
        ...
        logging.warning('Tentativa de sync sem sheet disponível.')
        return False
    try:
        records = sheet.get_all_records()
        conn = get_db_connection()
...
def create_reservation(user_id, service, quantity=1):
    ...
    if stock < quantity:
        ...
        try:
            ...
            c.execute('SELECT id FROM waiting_list WHERE user_id = ? AND service = ?', (user_id, service_norm))
            if c.fetchone():
                conn.close()
                return False, f'Você já está na fila de espera para {service.title()}.'
...
def get_all_minis_from_db():
    conn = get_db_connection()
    if not conn:
        logging.error('Falha na conexão DB para get_all_minis_from_db')
        return []
    c = conn.cursor()
    try:
        ...
        logging.error(f'Erro ao buscar minis do DB: {e}')
        return []
    finally:
        conn.close()
...
def get_all_reservations():
    ...
    try:
        ...
        logging.error(f'Erro ao buscar reservas: {e}')
        return []
    finally:
        conn.close()
...
def get_waiting_list():
    ...
    try:
        ...
        logging.error(f'Erro ao buscar fila de espera: {e}')
        return []
    finally:
        conn.close()
...
<body>
    ...
    <main class="grid-container">
        {% for mini in minis %}
            <div class="thumbnail{% if mini['quantity'] == 0 %} esgotado{% endif %}">
                {% if mini['quantity'] == 0 %}<div class="esgotado-tag">ESGOTADO</div>{% endif %}
                <img src="{{ logo_url }}" alt="{{ mini['service'].title() }}" onerror="this.onerror=null;this.src='{{ logo_url }}';"> <!-- Placeholder image -->
                <div class="thumbnail-content">
                    <h3>{{ mini['service'].title() }}</h3>
                    <p class="quantity">Disponível: {{ mini['quantity'] }}</p>
                    <div class="action-buttons">
                        {% if mini['quantity'] > 0 %}
                            <a href="/reserve/{{ mini['service'] }}" class="btn btn-reserve">Reservar Agora</a>
                        {% else %}
                            <a href="/waiting/{{ mini['service'] }}" class="btn btn-waiting">Fila de Espera</a>
                            <a href="https://wa.me/{{ whatsapp_number }}?text=Olá, gostaria de saber sobre a fila de espera para {{ mini['service'].title() }}. Meu email: {{ session.get('email', 'anônimo') }}" class="btn btn-contact" target="_blank">Entrar em Contato</a>
                        {% endif %}
                    </div>
                </div>
            </div>
...
<head>
    ...
    <style>
        ...
        input, select { width: calc(100% - 22px); padding: 10px; margin-bottom: 15px; border: 1px solid #ddd; border-radius: 5px; }
        .btn { background: #007bff; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; display: block; width: 100%; box-sizing: border-box; }
        .flash { padding: 10px; margin: 10px 0; border-radius: 5px; }
        .flash.success { background: #d4edda; color: #155724; }
        .flash.error { background: #f8d7da; color: #721c24; }
        img.mini-image { max-width: 100%; height: auto; margin: 20px auto; display: block; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); }
        .back-link { display: block; text-align: center; margin-top: 20px; color: #007bff; text-decoration: none; }
    </style>
</head>
<body>
    <h1>Reservar {{ service.title() }}</h1>
    <img src="{{ logo_url }}" alt="{{ service.title() }}" class="mini-image"> <!-- Usando LOGO_URL como placeholder -->
    {% with messages = get_flashed_messages(with_categories=true) %}
        {% if messages %}
            {% for category, message in messages %}
                <div class="flash {{ category }}">{{ message }}</div>
    ...
    {% if not session.user_id %}
        <p style="text-align: center;">Faça <a href="/login">login</a> para reservar.</p>
    {% else %}
        <form method="POST">
...
<head>
    ...
    <style>
        ...
        .btn { padding: 5px 10px; margin: 2px; border-radius: 3px; text-decoration: none; color: white; display: inline-block;}
        ...
        form input, form select, form textarea { width: calc(100% - 12px); padding: 5px; margin: 5px 0; border: 1px solid #ddd; border-radius: 3px; }
        form button { padding: 8px 15px; background: #007bff; color: white; border: none; border-radius: 3px; cursor: pointer; }
        .admin-nav a { margin-right: 15px; font-weight: bold; color: #007bff; text-decoration: none; }
...
<body>
    ...
    <p>Reservas Pendentes: {{ pending_res|length }}</p>
    <p>Total de Miniaturas em Estoque: {{ total_stock }}</p>
...
<head>
    ...
    <style>
        ...
        .btn { background: #007bff; color: white; padding: 10px; width: 100%; border: none; border-radius: 5px; cursor: pointer; display: block; box-sizing: border-box; }
...
<head>
    ...
    <style>
        ...
        form { background: white; padding: 20px; border-radius: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
        input { width: calc(100% - 22px); padding: 10px; margin: 10px 0; border: 1px solid #ddd; border-radius: 5px; }
        .btn { background: #28a745; color: white; padding: 10px; width: 100%; border: none; border-radius: 5px; cursor: pointer; display: block; box-sizing: border-box; }
        .flash { padding: 10px; margin: 10px 0; border-radius: 5px; text-align: center; }
        .flash.success { background: #d4edda; color: #155724; }
        .flash.error { background: #f8d7da; color: #721c24; }
        p { text-align: center; margin-top: 15px; }
...
<head>
    ...
    <style>
        ...
        .btn { background: #ffc107; color: #000; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block; }
    </style>
</head>
<body>
...
def home():
    try:
        ...
        logging.error(f'Erro na rota home: {e}')
        flash('Erro ao carregar a página inicial. Tente novamente mais tarde.', 'error')
        return render_template_string(HOME_TEMPLATE, minis=[], logo_url=LOGO_URL, whatsapp_number=WHATSAPP_NUMBER, total_stock=0)
...
def reserve(service):
    ...
    if stock == 0:
        flash(f'Estoque esgotado para {service.title()}. Você foi adicionado à fila de espera.', 'error')
        logging.info(f'Redirect de reserva para {service_norm} - estoque 0. Adicionando à fila.')
    ...
    if request.method == 'POST':
        try:
            ...
            if quantity <= 0:
                flash('Quantidade inválida. Deve ser maior que zero.', 'error')
                return render_template_string(RESERVE_TEMPLATE, service=service, stock=stock, logo_url=LOGO_URL)
            
            if quantity > stock:
                flash(f'Quantidade inválida. Máximo disponível: {stock}', 'error')
                return render_template_string(RESERVE_TEMPLATE, service=service, stock=stock, logo_url=LOGO_URL)
            ...
            logging.error(f'Erro inesperado na reserva de {service_norm}: {e}')
...
def waiting(service):
    ...
    conn = get_db_connection()
    if not conn:
        flash('Erro interno no banco de dados ao adicionar à fila.', 'error')
        return redirect(url_for('home'))
    c = conn.cursor()
    try:
...
def admin_backup_csv():
    ...
    try:
        ...
        c.execute('SELECT * FROM users')
        users_headers = [description[0] for description in c.description]
        ...
        c.execute('SELECT * FROM reservations')
        res_headers = [description[0] for description in c.description]
        ...
        c.execute('SELECT * FROM stock')
        stock_headers = [description[0] for description in c.description]
        ...
        c.execute('SELECT * FROM waiting_list')
        waiting_headers = [description[0] for description in c.description]
        writer.writerow(waiting_headers)
        writer.writerows(c.fetchall())
